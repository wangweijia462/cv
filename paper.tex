...
\linespread{1.5}  % 1.5 line spacing for DPE journal submission
...
\begin{abstract}
\noindent
We build a four-region New Keynesian DSGE model to quantify the global welfare costs of a Strait of Hormuz blockade, integrating trade, energy, financial, and stabilization channels. The financial linkage matrix is validated with a structural VAR(1) on four historical Middle East conflict episodes. A complete blockade collapses Iran's GDP by 27.2 percent and spikes global oil prices by 63 percent. Financial frictions account for 4.88 percentage points of cumulative global welfare loss---exceeding the entire trade-channel effect---while conventional monetary policy moves global welfare by less than 0.11 percentage points across all Taylor-rule regimes. China's manufacturing capacity, not its strategic petroleum reserve, is the binding stabilization margin: reserve releases exhaust their physical ceiling by quarter one, whereas doubling the manufacturing response saves 0.70 percentage points of China's own welfare. Supply-chain resilience and macroprudential policy, not central-bank activism, are the correct levers for geopolitical rare disasters.
...
\paragraph{Related literature.} Four strands of prior work inform our analysis. \textit{The economics of war and conflict.} Wars reduce war-site GDP by roughly 10 percent on average; third-country spillovers flow primarily through bilateral trade exposure \citep{federle2026price}. Reduced-form local projections identify these magnitudes cleanly but cannot decompose them into energy, financial, and supply-chain components, nor can they vary policy counterfactually. \citet{abadie2003economic} and \citet{barro2006rare} develop the foundational methodology---synthetic control and rare-disaster pricing---that we extend into a multi-channel structural framework. \textit{DSGE models of geopolitical risk.} \citet{caldara2022measuring} show that geopolitical risk shocks reduce investment and output significantly and that transmission is concentrated in financial markets rather than goods trade---the same channel ordering our model confirms. \citet{bodenstein2011oil} develop the two-country open-economy DSGE with oil that we extend to four regions; their calibration of short-run oil elasticities ($|\varepsilon_d| = 0.05$, $\varepsilon_s = 0.10$) carries directly into our oil-price equation~(\ref{eq:oil_price}). \textit{Oil shocks and the macroeconomy.} \citet{hamilton1983oil} establishes that supply disruptions cause recessions; \citet{kilian2009not} identifies the mechanism as supply- versus demand-driven; \citet{blanchard2007macroeconomic} document that energy-intensity heterogeneity matters for pass-through differentials, which we capture through region-specific $\alpha_{e,i}$ parameters. \textit{China's global stabilization role.} \citet{zhang2020china} analyze China's SPR in a DSGE and find releases effective at dampening oil-price volatility. \citet{helveston2019china} document that China's solar supply chain generates cost-reduction externalities---the same logic our manufacturing stabilization channel formalizes in a crisis context. \citet{longstaff2011financial} and \citet{rey2015dilemma} provide the empirical foundation for the financial contagion matrix.

Section~\ref{sec:facts} documents the stylized facts motivating the scenario. Sections~\ref{sec:model}--\ref{sec:calibration} present the model and calibration. Section~\ref{sec:results} reports baseline results. Section~\ref{sec:robustness} presents robustness checks. Section~\ref{sec:conclusion} draws policy implications.
...
\paragraph{Fact 1: Hormuz dominates global oil transit with no close substitute.} Figure~\ref{fig:chokepoints} ranks the world's major oil-transit chokepoints by daily volume. The Strait of Hormuz carries 21 million barrels per day---approximately 20 percent of global petroleum trade---roughly 30 percent more than the next-largest chokepoint (the Strait of Malacca at 16 mbd) and more than double the Suez Canal. Critically, it has no practical bypass: the Abqaiq--Yanbu pipeline can redirect at most 5 mbd of Saudi crude, leaving 16 mbd with no alternative route. This asymmetry means a blockade cannot be circumvented at scale within the time horizon of the shock---a modeling assumption our scenario embeds directly.
...
\begin{minipage}{\textwidth}
\footnotesize \textit{Notes:} Panel~A: PPP-adjusted GDP shares, 2022. Panel~B: Oil trade exposure as percent of GDP. Panel~C: Stabilization capacity proxies (normalized to China = 1.0; absolute values labeled). Source: IMF \textit{World Economic Outlook} (2023); World Bank WDI (2023); IEA \textit{Oil Market Report} (2023); IRENA (2023); WTO Trade Statistics.
\end{minipage}
\end{figure}

\paragraph{Fact 5: Middle East conflicts generate sovereign spread contagion far beyond bilateral trade exposure.} Figure~\ref{fig:spreadepisodes} plots sovereign spread deviations around four historical conflict episodes: the Iran--Iraq War (1980Q3), the Gulf War (1990Q3), the Iraq War (2003Q1), and the 2019 Hormuz tanker incidents (2019Q2). In every episode, sovereign spreads in the ROW aggregate (GDP-weighted Germany, France, UK, and Japan) rose substantially---averaging 30 percent of the Iran spread increase---despite minimal bilateral trade linkages with Iran or the Gulf. The speed of contagion (spreads move within one to two quarters of onset) and its persistence (elevated for four to eight quarters) are inconsistent with a pure trade-channel explanation and point to portfolio-rebalancing and risk-appetite channels as the primary transmission mechanism. These patterns motivate the $4 \times 4$ financial contagion matrix at the core of our model, and we validate its entries against VAR estimates of these same episodes in Section~\ref{sec:var}.
...
\footnotesize \textit{Notes:} Basis points deviation from four-quarter pre-event mean. Iran (solid) and ROW average (dashed). Source: JP Morgan EMBI+ database; Hilscher and Nosbusch (2010); Bloomberg sovereign CDS data (post-2003).
\end{minipage}
\end{figure}
...
\label{eq:oil_price}
\end{equation}
where $\varepsilon_d = -0.05$ and $\varepsilon_s = 0.10$ are short-run demand and supply elasticities \citep{hamilton2009understanding}; $\text{SPR}_t \geq 0$ denotes aggregate strategic reserve releases as a dimensionless fraction of global supply; and $\Delta D_t = -\mu_d \hat{y}^{\text{glob}}_t$ is the proportional change in global oil demand relative to steady-state supply, with $\mu_d = 0.40$ the income elasticity of oil demand. All three numerator terms are dimensionless fractions of global supply: $\Delta S_t$ is the supply shortfall share (equation~\ref{eq:oil_supply}), $\text{SPR}_t$ offsets that shortfall, and $\Delta D_t < 0$ during a recession, partially reducing the net supply gap. The denominator converts this net fractional gap into a price-level response via the standard inverse-elasticity formula.
...
\paragraph{SPR Intertemporal Stock Constraint.} The SPR is a \emph{stock} variable that depletes with each quarterly drawdown; releases are bounded above by the remaining stock:
\begin{equation}
R_t = R_{t-1} - \text{SPR}^{\text{China}}_t, \qquad R_0 = \bar{R}, \qquad 0 \leq \text{SPR}^{\text{China}}_t \leq R_{t-1}
\label{eq:spr_stock}
\end{equation}
where $R_t$ denotes remaining reserves (expressed as a fraction of quarterly oil imports) and $\bar{R} \approx 1$ corresponds to China's initial endowment of approximately 90 days ($\approx$1 quarter) of import cover. Under the baseline SPR path \eqref{eq:spr}, the cumulative drawdown over eight quarters is $\sum_{t=1}^{8}\text{SPR}^{\text{China}}_t \approx 4.9\%$ of quarterly imports, exhausting only about 5\% of $\bar{R}$; the stock constraint \eqref{eq:spr_stock} therefore does not bind at any point in the baseline scenario. The binding constraint is instead the quarterly \emph{flow-rate ceiling} $\bar{f} = 0.008$ (0.8\% of quarterly imports per period), which reflects the physical capacity of China's petroleum pipeline and port infrastructure to withdraw and distribute reserves.
...
\subsection{Financial Contagion Amplifies Third-Country Losses by 3.45\texorpdfstring{$\times$}{x}}
\label{sec:financial}
...
\begin{align}
\text{SPR}_t &= \rho_s \, \text{SPR}_{t-1} + \phi^s_p \max(p^{\text{oil}}_t - \bar p, 0) + \phi^s_y \max(-y^{\text{glob}}_{t-1}, 0), \label{eq:spr_rule} \\
\text{REN}_t &= \rho_r \, \text{REN}_{t-1} + \phi^r_p (p^{\text{oil}}_t - \bar p), \label{eq:ren_rule} \\
\text{MFG}_t &= \rho_m \, \text{MFG}_{t-1} + \phi^m_d \, D^{\text{trade}}_t + \phi^m_y \max(-y^{\text{glob}}_{t-1}, 0). \label{eq:mfg_rule}
\end{align}
where $D^{\text{trade}}_t$ is the global trade-disruption index and $\bar p$ is the long-run oil-price target. Reaction coefficients are set so that the rule replicates China's actual SPR-release behavior during the 2022 energy crisis. The endogenous SPR rule \eqref{eq:spr_rule} reaches its quarterly \emph{flow-rate ceiling} $\bar{f} = 0.008$ from the first quarter onward under any plausible parameterization; as established by the stock constraint \eqref{eq:spr_stock}, cumulative drawdown over eight quarters amounts to only $\approx$4.9\% of $\bar{R}$, so the stock is far from exhausted---the binding constraint is the extraction \emph{rate}, not the stock size. Additional reserves therefore have zero marginal value in this scenario. Doubling the manufacturing reaction coefficient $\phi^m_d$ in \eqref{eq:mfg_rule}, by contrast, reduces China's own welfare loss by 0.70 percentage points and reduces global welfare loss by 0.18 percentage points, identifying manufacturing capacity as the policy-relevant margin. Figure~\ref{fig:endogenous} reports the SPR and manufacturing paths under the deterministic baseline and the endogenous rule.
...
\begin{minipage}{0.9\textwidth}
\vspace{0.3em}
\small \textit{Notes:} Top row compares deterministic (baseline) and endogenous rule-based paths for SPR releases and manufacturing expansion. Bottom row plots the endogenous reaction functions \eqref{eq:spr_rule} and \eqref{eq:mfg_rule}. The SPR series binds at the capacity ceiling (dashed line) in quarters 1--8 under the endogenous rule.
\end{minipage}
\end{figure}
...
\begin{minipage}{0.85\textwidth}
\vspace{0.3em}
\small \textit{Notes:} Heat-map comparison of the hand-calibrated linkage matrix $\mathbf{L}$ (left panel) and the VAR(1) off-diagonal transmission coefficients estimated on four historical Middle East conflict episodes (right panel). Diagonal entries are set to zero in both panels to facilitate direct visual comparison of cross-regional transmission; Iran's VAR-estimated own-persistence is 0.598 (raw pass-through), reported in Table~\ref{tab:var}. Off-diagonal entries are spread-to-spread transmission coefficients. The two matrices are quantitatively close: maximum absolute deviation 0.075 (Iran$\to$ROW cell).
\end{minipage}
\end{figure}
