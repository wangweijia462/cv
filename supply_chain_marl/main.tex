...
\paragraph{Why CCD contributes marginally in S2 (Supply Interruption).}
In S2, CCD-MAPPO achieves $3945.7$ vs.\ $3938.8$ for MAPPO w/o CCD---a difference of
$6.9$ return units ($0.18\%$), well below statistical significance ($p=0.43$, Wilcoxon).
This near-zero margin is structurally analogous to S3: supply interruptions (one supplier
going offline) create a hard capacity constraint that \emph{no amount of data capability
can overcome}.  When the upstream supply is physically unavailable, the CCD bonus in
the reward does not help agents coordinate better; it only adds noise to the ordering
signal.  CCD's role is to improve \emph{information-processing coordination}; it has
near-zero marginal value when the binding bottleneck is a physical supply constraint
rather than an information asymmetry.  This limitation is consistent with the
information-theoretic framing of Proposition~\ref{prop:info} and further narrows the
operating conditions under which CCD-MAPPO offers the largest gains.

\paragraph{S3 anomaly.}
In S3, CCD-MAPPO achieves $-7206.4$ vs.\ $-7029.3$ for MAPPO w/o CCD, a $2.5\%$
disadvantage within two standard deviations ($p=0.21$, Wilcoxon, not significant).  The
mechanism is the same as S2: the lead-time multiplier $\kappa=2$ is a structural logistics
constraint, not an information-processing bottleneck.  The CCD bonus $\lambda_S\CCD_{i,t}$
in Eq.~\eqref{eq:reward} creates a slight over-ordering incentive to maintain high CCD---a
mismatch when the bottleneck is purely logistical.  A scenario-adaptive $\lambda_S$
schedule (reducing $\lambda_S$ when logistics disruption signals are detected) is a
natural remedy, left for future work.
