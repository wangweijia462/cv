...
the unique monotone, symmetric, balance-maximizing scalar in its function class
...
\begin{enumerate}[label=(\roman*),itemsep=2pt]
  ...
  \item Among all functions satisfying the properties in Lemma~\ref{lem:ccd} (symmetry,
    monotonicity, balance-sensitivity, non-linearity), $\CCD$ is the unique member of
    the coupling-coordination family $\sqrt{C(B,D)\cdot T(B,D)}$ that reduces to a
    monotone function of $B$ when $B=D$, making it a canonical one-dimensional
    sufficient statistic for the balance dimension.
...
(ii) Let $\mathcal{F}$ be the class of functions satisfying Lemma~\ref{lem:ccd}(i)--(iv).
The coupling-coordination family $\CCD(B,D)=\sqrt{C(B,D)\cdot T(B,D)}$ is the unique
member of $\mathcal{F}$ derived from the product of the coupling degree (harmonic/arithmetic
mean ratio, $C=2\sqrt{BD}/(B+D)$) and the synergy index ($T=\frac12(B+D)$).  Uniqueness
within this construction follows because $C$ and $T$ are the only functions of $(B,D)$
satisfying: $C=1$ iff $B=D$; $T$ is linear; $\CCD$ reduces to $B$ on the diagonal $B=D$.
...
Figure~\ref{fig:tr} shows the mean $T_R$ per method (bar chart; std.\ deviations reported
in the caption) across all 100 rollouts.  The key observation is that CCD-MAPPO achieves
the lowest mean $T_R$ within the CTDE class and also the lowest standard deviation
($\pm1.2$ vs.\ $\pm1.4$ for MAPPO w/o CCD), indicating more reliable recovery.
In supply-chain practice the tail of the recovery distribution often matters more than
the mean, since prolonged recoveries trigger cascading backorders and reputational
penalties; the reduction in std.\ is therefore operationally significant.
