...
\item \textbf{Representational efficiency of CCD.}  Under an empirically verified
  ...
  Section~\ref{sec:ablation}), we prove that CCD is the unique scalar feature in a
  natural function class that captures the non-additive interaction between data breadth
  and depth, which no linear combination $\alpha B+\beta D$ can recover (Proposition~1).
...
\begin{enumerate}[label=(\roman*),itemsep=2pt]
  ...
  \item \textbf{Expressiveness under constraint}: Despite the $\rho_{\mathrm{res}}$-bounded
    ...
    over the $(s,S)$ heuristic (Table~\ref{tab:main}), demonstrating that the constrained
...
(iii) Observed in Table~\ref{tab:main}: the mean return gap between CCD-MAPPO
