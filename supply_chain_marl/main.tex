...
\begin{enumerate}[label=(\roman*),itemsep=2pt]
  ...
  \item \textbf{Worst-case bound}: For any partially trained policy with arbitrary
    $\tilde{a}_{i,t}\in[0,1]$, the executed action satisfies
    $a_{i,t}\in[a^{\mathrm{ss}}_{i,t}-\rho_{\mathrm{res}},\;
    a^{\mathrm{ss}}_{i,t}+\rho_{\mathrm{res}}]\cap[0,1]$,
    bounding the deviation from the heuristic at \emph{every} training stage, not
...
(iii) Follows from (ii): for any $\tilde{a}\in[0,1]$, $(2\tilde{a}-1)\in[-1,1]$, so the
additive deviation $\rho_{\mathrm{res}}(2\tilde{a}-1)$ has magnitude at most
$\rho_{\mathrm{res}}$ regardless of $k$; the outer clip further enforces $[0,1]$
feasibility.  At $k=0$ the bound reduces to zero (Proposition~(i)), and it remains
$\rho_{\mathrm{res}}$ throughout training.
\end{proof}
