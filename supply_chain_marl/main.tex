...
methods handle poorly.  We propose \emph{CCD-MAPPO}, a Multi-Agent
...
and team reward.  We formally characterize CCD's representational efficiency: when the
optimal policy depends non-additively on the balance between breadth and depth
(Condition A2, empirically verified via ablation), no additive linear combination
$\alpha B+\beta D$ can capture the balance dimension that CCD encodes, and CCD provides
the canonical scalar in the coupling-coordination family that combines balance and level
(Proposition~1).  Second, we introduce a
...
Then:
\begin{enumerate}[label=(\roman*),itemsep=2pt]
  \item No additive function $h(B,D)=f(B)+g(D)$ can distinguish the two pairs in (A2),
    so any purely additive feature is representationally insufficient for $\pi^*$.
...
\begin{proof}
(i) If $h(B,D)=f(B)+g(D)$ and $B_1+D_1=B_2+D_2$, then $h$ cannot distinguish
the two pairs unless $f$ and $g$ jointly encode the balance; for a \emph{symmetric}
additive decomposition ($f=g$) this is impossible by construction, since $f(B_1)+f(D_1)
=f(B_2)+f(D_2)$ whenever $B_1+D_1=B_2+D_2$ and $f$ is linear.  For nonlinear $f$,
the distinction is possible in principle but requires learning it implicitly from data,
incurring higher sample complexity.
...
interquartile range from $[16,\,19]$ to $[14,\,16]$, representing a 37\% narrower
