...
\begin{enumerate}[label=(\roman*),itemsep=2pt]
  ...
  \item Among all functions satisfying the properties in Lemma~\ref{lem:ccd} (symmetry,
    ...
    $\sqrt{BD}$---capture balance but lack the level-sensitivity property: $\sqrt{BD}$
    is invariant to uniform scaling of both $B$ and $D$, so it cannot distinguish a
    high-capability balanced firm from a low-capability balanced firm.  $\CCD$ avoids
    this deficiency because $T$ encodes the level, making $\CCD$ representationally
    superior to alternatives in $\mathcal{F}$ under \emph{heterogeneous} capability
    endowments.
...
(ii) We do not claim uniqueness over the entire class $\mathcal{F}$; many functions in
$\mathcal{F}$ are non-additive and balance-sensitive.  The claim is that CCD is
\emph{canonically superior} among practically meaningful candidates because it separates
the balance and level dimensions into two interpretable sub-components ($C$ and $T$).
Formally, the geometric mean $g(B,D)=\sqrt{BD}$ satisfies all four Lemma~\ref{lem:ccd}
properties but is scale-equivariant: $g(\lambda B,\lambda D)=\lambda g(B,D)$ for all
$\lambda>0$, so it cannot distinguish $(B,D)=(0.2,0.2)$ from $(0.8,0.8)$ relative to a
fixed threshold.  By contrast, $\CCD(0.2,0.2)=0.2$ while $\CCD(0.8,0.8)=0.8$, because
$T=\frac12(B+D)$ encodes the level.  This level-sensitivity is essential in supply chains
where high-capability firms ($B\approx D\approx1$) can absorb larger coordination loads
than low-capability ones ($B\approx D\approx0.2$), even at the same balance ratio.
...
joint data capability (both broad data sources and deep analytics) to adapt its demand
...
\paragraph{Bounded gains in S2 and S3: physical constraints versus information bottlenecks.}
Two results we had not fully anticipated require explanation.  In S2 (Supply Interruption),
CCD-MAPPO achieves $-3945.7$ vs.\ $-3938.8$ for MAPPO w/o CCD---a difference of $6.9$
return units ($0.18\%$, $p=0.43$, Wilcoxon, not significant).  In S3 (Transportation Delay),
CCD-MAPPO is actually $2.5\%$ \emph{worse}: $-7206.4$ vs.\ $-7029.3$ ($p=0.21$, not
significant).  The shared explanation is that CCD encodes \emph{information-processing}
coordination; when the binding bottleneck is instead a \emph{physical} constraint---a
supplier that has gone offline, or a lead-time multiplier $\kappa=2$ baked into the
logistics layer---additional coordination signal cannot substitute for missing capacity.
In S3, the CCD bonus $\lambda_S\CCD_{i,t}$ in Eq.~\eqref{eq:reward} creates a slight
over-ordering incentive that is actively harmful when the bottleneck is logistical rather
than informational.  A scenario-adaptive $\lambda_S$ schedule that attenuates the CCD
weight when logistics disruption signals are detected is a natural remedy, which we leave
for future work; the key practical takeaway is that CCD-MAPPO's advantage is largest
precisely when information asymmetry---not physical capacity---is the limiting factor.
